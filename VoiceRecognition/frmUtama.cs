...
namespace VoiceRecognition
{
    public partial class frmUtama : MetroFramework.Forms.MetroForm
    {
        ...
        string[] data_personal =  { "who are you","what is your name", "GoSmart"
                                    , "hello", "what are you doing now", "where are you from"
        ...
        string[] data_todo = {  "facebook please", "google please","do you know CORTANA"
                                    ...
                                    , "shutdown please","print all comand","localhost please","please open notepad","how much culture in indonesia","clean list" };

        string[] data_personalD =  { "hello" };
         
        string[] data_todoD = { "google please","command prompt"
                                    , "task manager","please open notepad" };
        ...
        void recEngine_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            try
            {
                ...
                else if (e.Result.Text == data_todo[5])
                {
                    ...
                    synthesizer.SpeakAsync("ok my boss, please wait, i can open command prompt for you !");
                    Process.Start("cmd");
                }
                ...
                else if (e.Result.Text == data_todo[12])
                {
                    ...
                    frmlist.addData(t_ + "-> " + data_todo[12]);

                    synthesizer.SpeakAsync("Ok Zoha. Opening Notepad ");
                ...
                //}
                else if (e.Result.Text == data_todo[14])
                {
        ...
        {
            int lengtData = data_personalD.Length;
            frmlist.clearCommand();
            for (int i = 0; i < lengtData; i++)
            {
                frmlist.addData("- " + data_personalD[i]);
            }
            int lengthTodo = data_todoD.Length;

            for (int j = 0; j < lengthTodo; j++)
            {
                frmlist.addData(" - " + data_todoD[j]);
            }
        }
